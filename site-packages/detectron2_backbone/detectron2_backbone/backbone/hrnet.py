...
class HighResolutionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_branches): # 每个branch的输出shape和输入shape一样
            x[i] = self.branches[i](x[i])

        x_fuse = []
        for i in range(len(self.fuse_layers)):
            y = x[0] if i == 0 else self.fuse_layers[i][0](x[0]) # 第一个branch的输出不需要fuse
            for j in range(1, self.num_branches): # fuse_layer[0]: 192->96, [1]96->192且stride=2
                if i == j: # x[j]是fuse前各个分支原本的输入；
                    y = y + x[j]
                else:
                    y = y + self.fuse_layers[i][j](x[j])
            x_fuse.append(self.relu(y)) # 每个y是下一个stage相应branch的输入
...
class HRNet(Backbone):
    def __init__(self, cfg, **kwargs):
        ...
        self._out_feature_strides = {"stage1": 4, "stage2": 8, "stage3": 16, "stage4": 32}
    ...
    def forward(self, x): # (1, 3, 1200, 1600)
        x = self.conv1(x) # (1, 64, 600, 800)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x) # (1, 64, 300, 400)
        x = self.bn2(x)
        x = self.relu(x)
        x = self.layer1(x) # (1, 256, 300, 400)
        # x_list的两个元素分别经过stage2的两个分支，得到两个结果，再经过fuse，得到一个结果
        x_list = []
        for i in range(self.stage2_cfg['NUM_BRANCHES']): # 2
            if self.transition1[i] is not None: # transition1[0]: 256->96, stride=1; transition1[1]: 256->192, stride=2
                x_list.append(self.transition1[i](x))
            else:
                x_list.append(x) # x_list[0] (1, 96, 300, 400), x_list[1] (1, 192, 150, 200)
        y_list = self.stage2(x_list) # y_list[0] (1, 96, 300, 400), y_list[1] (1, 192, 150, 200)
        ...
        for i in range(self.stage3_cfg['NUM_BRANCHES']): # 3
            if self.transition2[i] is not None:
                ...
                x_list.append(y_list[i]) # x_list[0] (1, 96, 300, 400), x_list[1] (1, 192, 150, 200), x_list[2] (1, 384, 75, 100)
        y_list = self.stage3(x_list) # y_list[0] (1, 96, 300, 400), y_list[1] (1, 192, 150, 200), y_list[2] (1, 368, 75, 100)
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']): # 4
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i]) # x_list[0] (1, 96, 300, 400), x_list[1] (1, 192, 150, 200), x_list[2] (1, 368, 75, 100), x_list[3] (1, 768, 38, 50)
...
class LastLevelP6P7(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, in_feature="stage4"):
        super().__init__()
        self.num_levels = 2

...
def batched_soft_nms(
    ...
    offsets = idxs.to(boxes) * (max_coordinate + 1) # torch.to(tensor)重载，返回和目标tensor相同dtype和device的tensor
    # 用cls*一个最大值作为偏移，让所有不同类别的bbox不重合。
...
class SoftNMSer(object):
    def __init__(self, method, gaussian_sigma, linear_threshold, prune_threshold, use_batched_nms):
        self.method = method
    ...
    def __call__(self, boxes, scores, class_idxs):
        if self.use_batched_nms:
            ...
            return soft_nms(boxes, scores, self.method, self.gaussian_sigma, self.linear_threshold, self.prune_threshold)
...
def _soft_nms(
    ...
    while scores.numel() > 0:
        top_idx = torch.argmax(scores) # 取score最高的bbox
        idxs_out.append(idxs[top_idx].item()) # 被取出的框必定返回
        scores_out.append(scores[top_idx].item())

        top_box = boxes[top_idx]
        ious = pairwise_iou_func(box_class(top_box.unsqueeze(0)), box_class(boxes))[0] # 计算此框和剩下所有框的iou

        if method == "linear": # soft NMS with linear decay
            decay = torch.ones_like(ious)
            decay_mask = ious > linear_threshold
            decay[decay_mask] = 1 - ious[decay_mask]
        elif method == "gaussian": # soft NMS with exp decay
            decay = torch.exp(-torch.pow(ious, 2) / gaussian_sigma) # e^(-iou^2/σ)
        ...
        scores *= decay # scores每次乘以decay，score太低了就不要了。与top_box重叠的框衰减更快。

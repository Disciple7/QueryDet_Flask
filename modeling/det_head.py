...
class RetinaNetHead_3x3(nn.Module):
    def __init__(self, cfg, in_channels, conv_channels, num_convs, num_anchors):
        ...
        self.prior_prob       = cfg.MODEL.RETINANET.PRIOR_PROB
        ...
        self.cls_score = nn.Conv2d(channels, num_anchors * self.num_classes, kernel_size=3, stride=1, padding=1)
...
class RetinaNetHead_3x3_MergeBN(nn.Module):
    def __init__(self, cfg, in_channels, conv_channels, num_convs, num_anchors):
        ...
        channels = in_channels
        for i in range(self.num_convs):
            cls_layer = nn.Conv2d(channels, conv_channels, kernel_size=3, stride=1, padding=1, bias=False) #, activation=None, norm=None)
            bbox_layer = nn.Conv2d(channels, conv_channels, kernel_size=3, stride=1, padding=1, bias=False) # , activation=None, norm=None)
            # 上面两个本来是detectron2.layers.Conv2d，但是这里我把它改成了nn.Conv2d，因为我不想用detectron2的BN，而是用torch.nn.BatchNorm2d
            torch.nn.init.normal_(cls_layer.weight, mean=0, std=0.01)
            torch.nn.init.normal_(bbox_layer.weight, mean=0, std=0.01)
    ...
    def _forward_eval(self, features, lvl_start):
        ...
        return logits, bbox_pred # centerness这个应该是不用返回的。也不知道这个是啥。
